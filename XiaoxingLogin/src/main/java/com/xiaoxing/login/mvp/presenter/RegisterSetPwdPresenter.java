...
public class RegisterSetPwdPresenter extends BasePresenter<RegisterSetPwdContract.Model, RegisterSetPwdContract.View> {
    ...
    @Inject
    public RegisterSetPwdPresenter(RegisterSetPwdContract.Model model, RegisterSetPwdContract.View rootView) {
        super(model, rootView);
    }

    public void userRegister(String mobile_phone, String password1) {

        mModel.userRegister(mobile_phone, password1).subscribeOn(Schedulers.io())
//                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                ...
                .subscribe(new ErrorHandleSubscriber<UserRegister>(mErrorHandler) {
                    @Override
                    public void onNext(UserRegister userRegister) {
                        mRootView.userRegisterSuccess(userRegister);
                    }
                });
